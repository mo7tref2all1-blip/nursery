...
const DB_PATH = path_1.default.join(__dirname, '../../nursery.db');
...
if (teacherCount === 0) {
    const insertTeacher = db.prepare('INSERT INTO teachers (name, subject, phone, salary, join_date, status) VALUES (?, ?, ?, ?, ?, ?)');
...
if (studentCount === 0) {
    const insertStudent = db.prepare('INSERT INTO students (name, age, class_group, parent_name, parent_phone, join_date, status) VALUES (?, ?, ?, ?, ?, ?, ?)');
