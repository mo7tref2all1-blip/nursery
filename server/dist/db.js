...
async function addColIfMissing(conn, table, col, def) {
    const [rows] = await conn.execute(`SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
...
async function initDB() {
    ...
    `);
        ...
        const [nurseries] = await conn.execute('SELECT id FROM nurseries LIMIT 1');
        if (nurseries.length === 0) {
            await conn.execute("INSERT INTO nurseries (id, name, status, plan) VALUES (1, 'الحضانة الرئيسية', 'active', 'basic')");
            console.log('Default nursery created → الحضانة الرئيسية');
        }
        // ───── Seed admin (nursery_admin) ─────
        const [admins] = await conn.execute('SELECT id FROM users WHERE username = ?', ['admin']);
        if (admins.length === 0) {
            const hash = await bcryptjs_1.default.hash('admin123', 10);
            await conn.execute('INSERT INTO users (username, password_hash, role, nursery_id) VALUES (?,?,?,?)', ['admin', hash, 'nursery_admin', 1]);
            console.log('Default admin created  →  admin / admin123');
        }
        else {
            // Update existing admin: ensure role=nursery_admin and nursery_id=1 if NULL
            await conn.execute("UPDATE users SET role='nursery_admin', nursery_id=1 WHERE username='admin' AND (nursery_id IS NULL OR role='admin')");
        }
        // ───── Seed super_admin ─────
        const [superAdmins] = await conn.execute('SELECT id FROM users WHERE username = ?', ['superadmin']);
        if (superAdmins.length === 0) {
            const hash = await bcryptjs_1.default.hash('super123', 10);
            await conn.execute('INSERT INTO users (username, password_hash, role, nursery_id) VALUES (?,?,?,?)', ['superadmin', hash, 'super_admin', null]);
        ...
        // ───── Seed sample data ─────
        const [tc] = await conn.execute('SELECT COUNT(*) AS cnt FROM teachers');
        if (tc[0].cnt === 0) {
            await conn.execute('INSERT INTO teachers (name, subject, phone, salary, join_date, nursery_id) VALUES (?,?,?,?,?,?),(?,?,?,?,?,?),(?,?,?,?,?,?)', ['سارة أحمد', 'رياضيات', '0501234567', 5000, '2023-01-15', 1,
                'محمد علي', 'علوم', '0507654321', 5500, '2023-02-01', 1,
                'فاطمة حسن', 'لغة عربية', '0509876543', 4800, '2023-03-10', 1]);
        }
        const [sc] = await conn.execute('SELECT COUNT(*) AS cnt FROM students');
        if (sc[0].cnt === 0) {
            await conn.execute(`INSERT INTO students (name, age, class_group, parent_name, parent_phone, join_date, nursery_id) VALUES
